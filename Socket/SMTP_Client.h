...
public:
	__declspec(dllexport) std::string buildHeader(const std::string& sender, const std::string& recipient, const std::string& subject);
	__declspec(dllexport) std::string buildBody(const std::string& content);
	__declspec(dllexport) std::string base64EncodeFile(const std::string& filePath);
